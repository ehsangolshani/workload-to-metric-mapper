...
def augment_cpu_intensive_metrics(df: pd.DataFrame, interval: int = 60):
    ...
    df['cpu_utilization'] = (2 * (
            x + previous_cpu_utilization / 500 + previous_memory_utilization / 1000) ** 3) - (1 * (
            x + previous_cpu_utilization / 300 + previous_memory_utilization / 800) ** 2) + (
                                    3 * (x + previous_cpu_utilization / 100)) + 300
    df['memory_utilization'] = previous_memory_utilization / 400 + (x - previous_x) * 50 + 4500
